...
test("test deep json diff with json format", () => {
  const pathToFixtures = "/__fixtures__/testPack8/";
  const pathToFile1 = path.join(__dirname, pathToFixtures, "before.json");
  const pathToFile2 = path.join(__dirname, pathToFixtures, "after.json");
  const output = readFileSync(
    path.join(__dirname, pathToFixtures, "output.json"),
    "utf8"
  );
  expect(gendiff(pathToFile1, pathToFile2, "json")).toStrictEqual(
    JSON.parse(output)
  );
});
